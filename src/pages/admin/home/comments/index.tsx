...
export default function Page() {
    ...
    const tableHeaders = [
      { label: '頭像', value: 'avatar' },
      { label: '客戶名稱', value: 'customer_name' },
    ...
    const handleDeleteAction  = async (id: string) => {
      const res = await api({
        method: 'DELETE',
        url: `/admin/customer_comments/${id}`
      })
      if (res.code === 0) {
        return true
      }
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="max-sm:w-full sm:flex-1">
          ...
          <div className="mt-4 flex max-w-xl gap-4 flex-wrap">
            ...
            <div>
              <Button onClick={() => router.push('/admin/home/comments/sort')}>調整前台排序</Button>
            </div>
          </div>
        </div>
        <Button onClick={() => router.push('/admin/home/comments/creation')}>
          建立
        </Button>
      </div>
      <ul className="mt-4">
                      ...
                      <TableRow key={row.id}>
                          <TableCell>
                            <img
                              src={row.avatar}
                              alt=""
                              className="object-cover w-[48px] rounded-full aspect-[1/1]"
                            />
                          </TableCell>
                          <TableCell>
                            {row.customer_name}
                          ...
                          <TableCell>
                            <Dropdown>
                              ...
                              <DropdownMenu anchor="bottom end">
                                  <DropdownItem href={`/admin/home/comments/${row.id}/view`}>查看</DropdownItem>
                                  <DropdownItem href={`/admin/home/comments/${row.id}/edit`}>編輯</DropdownItem>
