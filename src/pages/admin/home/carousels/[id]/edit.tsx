      ...
      if (router.query.id) {
        getData().then((res) => {
          if (res.code === 0 && res.data) {
            ...
            setValue('url_open_type', data.url_open_type)
            if (imageMobileUploaderRef.current) {
              imageMobileUploaderRef.current.setList([{ id: '1', url: data.image_mobile }])
            }
            setValue('image_mobile', [data.image_mobile])
            if (imageDesktopUploaderRef.current) {
              imageDesktopUploaderRef.current.setList([{ id: '1', url: data.image_desktop }])
            }
            setValue('image_desktop', [data.image_desktop])
          }
        })
      }
    }, [router.query.id])
    ...
    <LayoutAdmin>
        <form onSubmit={handleSubmit(onSubmit)} className="mx-auto max-w-4xl">
            ...
            <div className="flex justify-end gap-4">
              ...
              <Button type="reset" plain onClick={() => router.push(`/admin/home/carousels/${router.query.id}/view`)}>
                查看
              </Button>
              <Button
